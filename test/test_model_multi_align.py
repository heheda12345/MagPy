...
from transformers.models.align.configuration_align import (
    AlignConfig,
    AlignTextConfig,
    AlignVisionConfig,
)
...
class AlignVisionEmbeddings(nn.Module):
    ...
    def __init__(self, config: AlignVisionConfig):
        ...
        self.batchnorm = nn.BatchNorm2d(self.out_dim,
...
class AlignVisionFinalBlockLayer(nn.Module):
    ...
    def forward(self, embeddings: torch.FloatTensor,
                hidden_states: torch.FloatTensor) -> torch.Tensor:
...
class AlignVisionBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.expand:
            self.expansion = AlignVisionExpansionLayer(config=config,
                                                       in_dim=in_dim,
    ...
    def __init__(self, config: AlignVisionConfig):
        ...
        for i in range(num_base_blocks):
            ...
            for j in range(round_repeats(config.num_block_repeats[i])):
                ...
                adjust_padding = (False if curr_block_num
                                  in config.depthwise_padding else True)
...
class AlignModel(AlignPreTrainedModel):
    ...
    def __init__(self, config: AlignConfig):
        ...
        self.temperature = nn.Parameter(
            torch.ones([]) * self.config.temperature_init_value)
    ...
    @replace_return_docstrings(output_type=AlignOutput,
                               config_class=AlignConfig)
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        ...
        logits_per_text = torch.matmul(text_embeds,
                                       image_embeds.t()) / self.temperature
    ...
    inputs = {
        'input_ids':
            torch.randint(low=0,
                          ...
                          dtype=torch.int64),
        'token_type_ids':
            torch.zeros((2, seq_len), dtype=torch.int64),
        'attention_mask':
            torch.ones((2, seq_len), dtype=torch.int64),
        'pixel_values':
            torch.randn([batch_size, 3, 289, 289])
    }
    inputs = (
        inputs["input_ids"],
        inputs["pixel_values"],
