...
def test_bn_function_factory_break(caplog):
    reset()
    conv = torch.nn.Conv2d(128, 128, (1, 1), 1, bias=False)
    norm = torch.nn.BatchNorm2d(128)
    relu = torch.nn.ReLU(inplace=True)
    inputs = [torch.randn((1, 64, 56, 56)), torch.randn((1, 64, 56, 56))]
    bn = _bn_function_factory(norm, relu, conv)
    expect = bn(*inputs)
    add_force_graph_break(get_next_frame_id(), 6)
    compiled = compile(_bn_function_factory)
    run_and_check_cache(compiled, [MISS], 2, caplog, norm, relu, conv)
    run_and_check_cache(compiled, [HIT, MISS], 3, caplog, norm, relu, conv)
